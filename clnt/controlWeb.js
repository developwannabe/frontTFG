function ControlWeb() {
            ...
            cr.obtenerLugares($.cookie("tkn"), function (data) {
                ...
                $("#solicitarRutaBtn").click(function (event) {
                    if (
                        ...
                        $("#bomberosRadio").prop("disabled", true);
                        $("#sanitariosRadio").prop("disabled", true);
                        $("#solicitarRutaBtn").addClass("hidden");
                        $("#calculandoButton").removeClass("hidden");
                        cr.solicitarRuta(
                            $.cookie("tkn"),
                            $("input[name=tipoEmergencia]:checked").val(),
                            $("#destinosSelect").val(),
    ...
    this.mostrarRuta = function (data) {
        if (data.mapa != null) {
            $("#mapaRes").attr("srcdoc", data.mapa);
            $("#mapaRes").removeClass("hidden");
        } else {
            ...
            $("#botDiv").append(`<a
                    ...
                    href="#"
                    title=""
                    class="w-[200px] text-white mt-4 sm:mt-0 bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 flex items-center justify-center"
                    role="button"
                >
                    Solicitar Ruta
                </a>`);
    ...
    this.verEvaluacion = function (id) {
        ...
        $("#centerContent").load("./clnt/evalOld/evaluacion.html", function () {
            ...
            cr.evaluacion($.cookie("tkn"), id, function (datos) {
                ...
                <div class="flex flex-row items-center"><h3 class="font-semibold text-gray-900 dark:text-white p-1">Evaluación realizada por: </h3>${
                    datos.result.user
                }</div>
                <div><h3 class="font-semibold text-gray-900 dark:text-white p-1">Magnitud del Terremoto: ${
                    datos.result.magnitude / 10
                }</h3></div>
                </div>
            `);
